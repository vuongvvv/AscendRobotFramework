...
Put User Update Registration
    [Arguments]    ${type_id}    ${merchant_id}    ${data}
    ${RESP}=    Put Request    ${GATEWAY_SESSION}    /notification/api/types/${type_id}/merchants/${merchant_id}/register    data=${data}    headers=&{gateway_header}
    Set Test Variable    ${RESP}
...
Get Merchant Id Test Data
    [Arguments]    ${type_id}=1    ${filter}=${EMPTY}
    Get All Merchant Notification Types    ${type_id}    ${filter}
    ${TEST_DATA_MERCHANT_ID}=    Get Property Value From Json By Index    .merchantId    0
